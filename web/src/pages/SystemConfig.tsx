import { type FC, useState, useEffect, useCallback } from 'react';
import {
  Table,
  Button,
  Modal,
  Form,
  Input,
  App,
  Typography,
} from 'antd';
import { EditOutlined } from '@ant-design/icons';
import type { ColumnsType } from 'antd/es/table';
import type { SystemConfig as SystemConfigType } from '@/types/config.ts';
import { getConfigs, updateConfig } from '@/api/configs.ts';
import { formatTime } from '@/utils/format.ts';
import { LIMIT_CONFIG_KEYS } from '@/utils/constants.ts';

const { Text } = Typography;

/** 编辑表单字段 */
interface EditFormValues {
  config_value: string;
}

/**
 * 系统配置页面
 * 功能: 展示所有系统配置项，支持 Modal 编辑
 */
const SystemConfig: FC = () => {
  const [loading, setLoading] = useState(false);
  const [configs, setConfigs] = useState<SystemConfigType[]>([]);
  const [editModalOpen, setEditModalOpen] = useState(false);
  const [editingConfig, setEditingConfig] = useState<SystemConfigType | null>(null);
  const [form] = Form.useForm<EditFormValues>();
  const { message } = App.useApp();

  /** 加载配置列表，排除限额相关的配置项（由限额管理页面管理） */
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const all = await getConfigs();
      const filtered = all.filter((c) => !LIMIT_CONFIG_KEYS.has(c.config_key));
      setConfigs(filtered);
    } catch {
      message.error('加载系统配置失败');
    } finally {
      setLoading(false);
    }
  }, [message]);

  useEffect(() => {
    loadData();
  }, [loadData]);

  /**
   * Modal 打开且待编辑配置就绪后，再设置表单值
   * 避免 destroyOnHidden 导致 Form DOM 不存在时调用 setFieldsValue 的警告
   */
  useEffect(() => {
    if (editModalOpen && editingConfig) {
      form.setFieldsValue({
        config_value: editingConfig.config_value,
      });
    }
  }, [editModalOpen, editingConfig, form]);

  /** 打开编辑弹窗 */
  const handleEdit = (record: SystemConfigType) => {
    setEditingConfig(record);
    setEditModalOpen(true);
  };

  /** 提交编辑 */
  const handleEditSubmit = async () => {
    if (!editingConfig) return;
    try {
      const values = await form.validateFields();
      setLoading(true);
      await updateConfig({
        id: editingConfig.id,
        config_value: values.config_value,
      });
      message.success('配置更新成功');
      setEditModalOpen(false);
      setEditingConfig(null);
      await loadData();
    } catch (e: unknown) {
      if (e && typeof e === 'object' && !('errorFields' in e)) {
        message.error('操作失败');
      }
    } finally {
      setLoading(false);
    }
  };

  /** 表格列定义 */
  const columns: ColumnsType<SystemConfigType> = [
    {
      title: '配置项',
      dataIndex: 'config_key',
      key: 'config_key',
      width: 240,
      render: (val: string) => <Text code>{val}</Text>,
    },
    {
      title: '值',
      dataIndex: 'config_value',
      key: 'config_value',
      width: 200,
      render: (val: string) => (
        <Text strong style={{ color: '#1677ff' }}>
          {val}
        </Text>
      ),
    },
    {
      title: '说明',
      dataIndex: 'description',
      key: 'description',
      ellipsis: true,
    },
    {
      title: '更新时间',
      dataIndex: 'updated_at',
      key: 'updated_at',
      width: 170,
      render: (val: string) => formatTime(val),
    },
    {
      title: '操作',
      key: 'action',
      width: 80,
      fixed: 'right',
      render: (_: unknown, record: SystemConfigType) => (
        <Button
          type="link"
          size="small"
          icon={<EditOutlined />}
          onClick={() => handleEdit(record)}
        >
          编辑
        </Button>
      ),
    },
  ];

  return (
    <div>
      <Table<SystemConfigType>
        columns={columns}
        dataSource={configs}
        rowKey="id"
        loading={loading}
        pagination={false}
        scroll={{ x: 900 }}
        size="middle"
      />

      {/* 编辑配置弹窗 */}
      <Modal
        title={
          editingConfig
            ? `编辑配置: ${editingConfig.config_key}`
            : '编辑配置'
        }
        open={editModalOpen}
        onOk={handleEditSubmit}
        onCancel={() => {
          setEditModalOpen(false);
          setEditingConfig(null);
        }}
        okText="保存"
        cancelText="取消"
        confirmLoading={loading}
        destroyOnHidden
      >
        {editingConfig && (
          <div style={{ marginBottom: 16 }}>
            <Text type="secondary">{editingConfig.description}</Text>
          </div>
        )}
        <Form
          form={form}
          layout="vertical"
        >
          <Form.Item
            name="config_value"
            label="配置值"
            rules={[{ required: true, message: '请输入配置值' }]}
          >
            <Input placeholder="请输入配置值" />
          </Form.Item>
        </Form>
      </Modal>
    </div>
  );
};

export default SystemConfig;
